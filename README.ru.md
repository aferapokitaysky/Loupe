...
- **Показ потока TCP**: обе стороны диалога, собранные заново, текстом или в
  hex, с объёмом, копированием и сохранением. Направления показаны отдельно:
  сборка знает порядок байтов внутри каждого, но не то, как они чередовались во
  времени, — а выдумывать это значит подавать догадку за факт.
- Готовые фильтры захвата (веб, TLS, QUIC, HTTP, DNS, без ARP и широковещания)
  подставляются в поле фильтра, так что синтаксис BPF остаётся на виду.
- Копирование строки, любого из адресов и hex-дампа из контекстного меню.
- Чтение и запись классического `.pcap` — захваты открываются в Wireshark и
  tcpdump. `loupe capture.pcap` (или «Открыть с помощью») открывает файл сразу.
...
**Что можно сделать с запросом.** Повторить — он уходит прямо на сервер, а не
через сам прокси, иначе он записался бы дважды, — и ответ появляется в списке
рядом с исходным, с пометкой повтора. Скопировать как cURL, PowerShell или
`fetch()`: заголовки уровня соединения отбрасываются, а двоичное тело
описывается, а не вставляется. Скопировать или сохранить тело ответа в
раскодированном виде — на диск ложится то, что имел в виду сервер, а не gzip,
которым это ехало.

Переключатель «Ошибки» оставляет только 4xx, 5xx и запросы, которые остались без
ответа, а поиск по желанию заходит и внутрь тел запросов и ответов.
...
| Файл | Для чего |
|---|---|
| `Loupe-<версия>.exe` | Один файл: скачать и запустить. Без установки, без распаковки и без консоли за окном |
| `Loupe-<версия>-win-x64.zip` | Та же сборка архивом, вместе с лицензией и этим README |
...
| `Ctrl` + `R` | повторить выбранный запрос |
...
захват вместе с приложением, что Loupe хранит на этой машине и как это стереть,
и правила скрытия со сбросом. Окно к тому же открывается там, где его закрыли.
