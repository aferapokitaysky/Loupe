...
public partial class MainViewModel : ObservableObject, IDisposable
{
    ...
    private void DrainIncomingPackets()
    {
        ...
        RefreshHosts();
        if (IsFiltered) UpdateFilterSummary();
    ...
    private void ApplyFilter()
    {
        ...
        view.Filter = host is null && search.Length == 0
            ...
            : item => item is PacketRowViewModel row && Matches(row, host, search);

        UpdateFilterSummary();
    }

    private static bool Matches(PacketRowViewModel row, IPAddress? host, string search)
    {
